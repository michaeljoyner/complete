<footer class="main-footer">
    <p class="footer-tagline">"Complete living is about finding the balance between physical, emotional and mental health to sustain and enjoy a complete life"</p>
    <div class="footer-col">
        <h3>Be Social</h3>
        <div class="social-links">
            <a class="social-link-icon" href="https://twitter.com/2CompleteLiving">
                <img src="{{ asset('images/assets/twitter.png') }}" alt="twitter link">
            </a>
            <a class="social-link-icon" href="https://www.facebook.com/CompleteLiving">
                <img src="{{ asset('images/assets/fb.png') }}" alt="twitter link">
            </a>
            <a class="social-link-icon"
               href="ma&#105;l&#116;o&#58;&#115;t&#101;&#112;h&#106;&#111;y&#110;&#101;&#114;1&#56;&#64;&#103;mail&#46;&#99;&#111;&#109;">
                <img src="{{ asset('images/assets/email.png') }}" alt="twitter link">
            </a>
        </div>
    </div>
    <div class="footer-col">
        <h3>I love these Guys...</h3>
        <ul class="partners-list">
            <li><a href="https://www.facebook.com/WickedlyHealthyFood">wickedly healthy food</a></li>
            <li><a href="https://www.facebook.com/Healthy-Times-715651755127246/">healthy times</a></li>
            <li><a href="http://www.phatgirlsorders.wix.com/phat-girls">Phat Girls</a></li>
        </ul>
    </div>
    <div class="footer-col">
        <h3>Boring, but important</h3>
        <p>I am a registered member of the HPCSA (DT 0038318)</p>
        <p>My practice number is: 0454524</p>
        <p>All content is copyrighted &copy;</p>
    </div>
</footer>